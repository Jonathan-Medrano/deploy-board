...
export function extraerObjetos(sql) {
  ...
  const mod = sql.match(
    new RegExp(String.raw`\b(?:CREATE|ALTER)(?:\s+OR\s+ALTER)?\s+(PROCEDURE|PROC|FUNCTION|VIEW|TRIGGER)\s+` + ID, 'i')
  );
  if (mod) {
  ...
  for (const m of sql.matchAll(reMax)) {
  ...
  for (const m of sql.matchAll(reAdd)) {
  ...
  for (const m of sql.matchAll(reFila)) {
  ...
  for (const m of sql.matchAll(reIdx)) {
