...
export async function descubrirAdjuntos(ado, iteracion) {
  ...
  if (!ids.length) return { scripts: [], wis: [], tasks: [] };

  const items = await ado.getWorkItems(ids);
  const scripts = [];
  const tasks = [];
  const wis = items.map((w) => {
    const f = w.fields || {};
    return {
      id: w.id,
    ...
    for (const a of sqls) {
      ...
      try {
        ...
        s = {
          ...
          objetos: [], sql: '', fuente: 'adjunto', contenedorId: w.id, responsables: {},
